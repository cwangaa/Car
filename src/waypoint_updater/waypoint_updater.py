...
class WaypointUpdater(object):
	...
	#print ("end sleep")
        ...
        y = self.y
        z = self.z
        if (self.wps is None) or (self.x is None) or (self.y is None) or (self.z is None): continue
        # TODO: Implement
        #if self.count <  3:
        #    print(msg)
        self.count += 1
       ...
       # if (self.count < 3):
        ...
        while True:
            if a < 4.0:
                self.wps[i].twist.twist.linear.x = 22.4 #10.0
            else:
                xw = self.wps[i].pose.pose.position.x
                yw = self.wps[i].pose.pose.position.y
                dx = self.wps[nexti].pose.pose.position.x-xw
                dy = self.wps[nexti].pose.pose.position.y-yw
